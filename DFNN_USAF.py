...
class WDSR(nn.Module):
    def __init__(self, num, down_sample_scale, args = args):
        ...
        if np.log2(down_sample_scale) == 0.0:
            down = [W_N(nn.Conv2d(args.features, args.features, kernel_size=3, stride=1, padding=1))] #args.num_LR, args.features  #args.num_LR
        else:
            down = [W_N(nn.Conv2d(args.features, args.features, kernel_size = 3, stride = 2, padding = 1))] + \
...
class FP_SR_WDSR(nn.Module):
    def __init__(self):
        ...
        up_1 = [W_N(nn.Conv2d(args.num_LR, args.features, kernel_size = 3, stride = 1, padding = 1)),
                nn.UpsamplingBilinear2d(scale_factor = 2)]
        self.up_1 = nn.Sequential(*up_1)
        
        up_2 = [W_N(nn.Conv2d(args.num_LR, args.features, kernel_size = 3, stride = 1, padding = 1)),
                nn.UpsamplingBilinear2d(scale_factor = 2)]
    ...
    def forward(self, tmp):
        ...
        #PHASE
        PHASE1 = self.PHASE1(y)
        PHASE2 = self.PHASE2(y)
