...
class FP_SR_WDSR(nn.Module):
    def __init__(self):
        ...
        self.skip_abs = nn.Sequential(W_N(nn.Conv2d(args.num_LR, 32, kernel_size = 3, padding = 1, stride = 1)),
                                      W_N(nn.Conv2d(32, 32 * args.scale ** 2, kernel_size = 5, padding = 2, stride = 1)),
                                      nn.PixelShuffle(args.scale))
        #PHASE
        self.PHASE1 = WDSR(num = 16, down_sample_scale = 1)
        self.PHASE2 = WDSR(num = 16, down_sample_scale = 4)
        self.skip_phase = nn.Sequential(W_N(nn.Conv2d(args.num_LR, 32, kernel_size = 3, padding = 1, stride = 1)),
                                        W_N(nn.Conv2d(32, 32 * args.scale ** 2, kernel_size = 5, padding = 2, stride = 1)),
                                        nn.PixelShuffle(args.scale))
